...
x_line_fix_length = 10 # 初始行间距 mm
y_fix_length      = 10 # 单个石块-Y向间距 mm
...
def pe_belgian_road_line(n_line, x_init, x_set, y_set, z_set, y_edge_set, x_edge_set):
	...
	for n in range(n_line):
		...
		y_loc += (y_edge + y_edge_old)/2 + y_fix_length
		y_edge_old = y_edge
...
def pe_belgian_road(num, n_line, x_init, x_set, y_set, z_set, y_edge_set, x_edge_set):
	...
	# x_line_fix_length = 1 # 初始行间距 mm !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

	for n in range(num):
		x_init_loc += x_edge + x_line_fix_length
		if divmod(n,2)[-1]==1:
			n_line_new = n_line
		else:
			n_line_new = n_line-1
...
def test_belgian2():
	...
	y_set = [140,50]	# 石块y向长度 
	z_set = [25.2,10]	# 石块z向长度 
	y_edge_set = [30,3]	# 边界y向长度 
	x_edge_set = 30 		# 边界x向长度
	belgian_len = 30000 # 比利时路长度 mm
	width = 5000 		# 比利时路宽度 mm

	x_start_belgian = 10000
	x_front = 100000
	x_rear 	= 20000
