...
def sim_brake_single(params):
    ...
    for line, comment, req_unit in zip(data, comments, req_units):
        if req_unit == 'rad':
            result_total_dic[comment] = [n*180/math.pi for n in line]
            continue
        result_total_dic[comment] = line

    # start_dic = parase_dic_loc(result_total_dic, brake_start_loc)
    # end_dic   = parase_dic_loc(result_total_dic, -1)
    
    result_dic = parase_dic_init_loc(result_total_dic, brake_start_loc)
    result_dic['x_acc'] = result_total_dic['x_acc'][brake_start_loc:]       # 加速度, 不取相对值
    result_dic['velocity'] = result_total_dic['velocity'][brake_start_loc:] #   速度, 不取相对值
    value_end_dict = parase_dic_loc(result_dic, -1)
    ...
    return value_end_dict, result_dic, samplerate, result_total_dic
...
def sim_cur_brake(params):
    ...
    def ite_run(params, last_acc, gain=gainit, n_ite=maxit):
        ...
        for calc_n in range(n_ite):
            ...
            value_end, result, samplerate, result_total = sim_brake_single(params)
        ...
        return value_end, result, cur_brake, result_total
    ...
    for velocity in velocity_list:
        ...
        value_end_tgt, result_tgt, brake_tgt, result_total_tgt = ite_run(params, last_acc)
        
        results['v'+str(int(velocity))] = {
            "params": copy.deepcopy(params),      # 对应参数
